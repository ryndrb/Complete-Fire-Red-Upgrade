	...
	for (u32 bankDef = 0; bankDef < gBattlersCount; ++bankDef)
	{
		...
		else 
		{
			critChance  = 2 * ((gBattleMons[gBankAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
						...
						+ (atkAbility == ABILITY_BLADEMASTER && gSpecialMoveFlags[gCurrentMove].gSwordMoves)
...
u8 TypeCalc(u16 move, u8 bankAtk, u8 bankDef, struct Pokemon* monAtk, bool8 CheckParty)
{
	...
	else if (gSpecialMoveFlags[gCurrentMove].gPowderMoves && !IsAffectedByPowder(bankDef))
	{
		flags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
	}
...
u8 AI_SpecialTypeCalc(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	else if (gSpecialMoveFlags[gCurrentMove].gPowderMoves && !IsAffectedByPowderByDetails(defType1, defType2, defType3, defAbility, defEffect))
	{
		flags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
	}
...
//Stat Buffs - Target
	...
	&& (data->defBuff != 6 || data->spDefBuff != 6))
	{
		if (gCritMultiplier > BASE_CRIT_MULTIPLIER)
		{
			if (data->defBuff < 6)
...
static u16 AdjustBasePower(struct DamageCalc* data, u16 power)
{
	...
	switch(data->atkAbility) {
		...
		//1.2x Boost
			if(SpeciesHasStriker(data->atkSpecies)){
		...
		//1.2x boost, gives +crit ratio to sword moves check above 
			if (gSpecialMoveFlags[move].gSwordMoves)
				power = (power * 12) / 10;
			break;
...
{
	u8 bankAtk, bankDef;
	u8 power = gBattleMoves[MOVE_HIDDENPOWER].power;
